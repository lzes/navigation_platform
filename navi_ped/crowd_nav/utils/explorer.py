...
class Explorer(object):
    ...
    def run_k_episodes(self, k, phase, update_memory=False, imitation_learning=False, episode=None, epoch=None,
        ...
        for i in range(k):
            ...
            rewards = []
            while not done:
                action = self.robot.act(ob)  # policy.predict(state)
                ob, reward, done, info = self.env.step(action)
                states.append(self.robot.policy.last_state)  # (robot_state, human_states)
    ...
    def update_memory(self, states, actions, rewards, imitation_learning=False):
        """
        memory更新整个episode内的所有 (state, value)。
        state: [(robot_state, human_states), ...]
        value: 一个数。在IL阶段,等于从当前state开始的reward的累加和; 在RL阶段,等于当前state的reward加上下一个state的value
        """
        ...
        for i, state in enumerate(states[:-1]):
            ...
            if imitation_learning:
                ...
                value = sum([pow(self.gamma, (t - i) * self.robot.time_step * self.robot.v_pref) * reward *
                             (1 if t >= i else 0) for t, reward in enumerate(rewards)])  # 从i开始的reward的累加和
            else:
