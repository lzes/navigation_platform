...
class MPRLTrainer(object):
    ...
    def optimize_epoch(self, num_epochs):  # 用于模仿学习,直接学(state, value)对
    ...
    def optimize_batch(self, num_batches, episode):  # 用于强化学习，Double DQN
        if self.v_optimizer is None:
            raise ValueError('Learning rate is not set!')
        if self.data_loader is None:
            self.data_loader = DataLoader(self.memory, self.batch_size, shuffle=True)
...
class VNRLTrainer(object):
    ...
    def optimize_epoch(self, num_epochs):  # 只训练了价值函数
    ...
    def optimize_batch(self, num_batches, episode=None):  # 训练至多一个epoch
        if self.optimizer is None:
            raise ValueError('Learning rate is not set!')
        if self.data_loader is None:
            self.data_loader = DataLoader(self.memory, self.batch_size, shuffle=True, collate_fn=pad_batch)
...
def pad_batch(batch):
    """
    args:
        batch - list of (states, values, rewards, next_states)
    return:
