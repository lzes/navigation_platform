...
class RGL(nn.Module):
    def __init__(self, config, robot_state_dim, human_state_dim):
        ...
        self.num_layer = num_layer  # GCN layers
        self.X_dim = X_dim
        self.layerwise_graph = layerwise_graph  # 是否每层GCN都重新计算邻接矩阵
        ...
        self.Ws = torch.nn.ParameterList()  # X_dim -> embedding_dim -> final_state_dim
    ...
    def compute_similarity_matrix(self, X):
        # [batch_size, num_human+1, X_dim]
        if self.similarity_function == 'embedded_gaussian':
            A = torch.matmul(torch.matmul(X, self.w_a), X.permute(0, 2, 1))  # 就是QKV的方式，这里的Q和V是X, K是X@w_a
        ...
        return normalized_A  # [batch_size, num_human+1, num_human+1]
    ...
    def forward(self, state):
        ...
        return:
            next_H: [batch_size, num_human+1, X_dim]
        """
        ...
        X = torch.cat([robot_state_embedings, human_state_embedings], dim=1)  # [batch_size, num_human+1, X_dim]
        ...
        return next_H  # [batch_size, num_human+1, X_dim]
