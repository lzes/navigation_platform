...
class StatePredictor(nn.Module):
    ...
    def forward(self, state, action, detach=False):
        ...
        state_embedding = self.graph_model(state)  # [batch_size, num_human+1, X_dim]
        if detach:
        ...
        if action is None:
            ...
            next_robot_state = self.compute_next_state(state[0], action)  # [batch_size, 1, robot_state_dim]
        next_human_states = self.human_motion_predictor(state_embedding)[:, 1:, :]  # [batch_size, num_human, human_state_dim]
