...
class ModelPredictiveRL(Policy):
    ...
    def get_model(self):  # 返回的是价值函数，没返回状态预测函数
    ...
    def V_planning(self, state, depth, width):
        ...
        returns:
            max_return: the value of the current state 一个数值
            max_traj: 树搜索路径[(state, action, reward), (state, action, reward), ...]

        """

        current_state_value = self.value_estimator(state)  # [batch_size, 1]
        if depth == 1:
